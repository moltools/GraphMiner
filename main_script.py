...
import csv
import time
start = time.time()  # should be moved to main() function
...
infile = load_data(1, ',')  # would use argparse, or index argv here directly and not send index as parameter
                            # make sure parsing of any command line parameter happens right at start of script in main function
...
def main(logger: Logger) -> None:
    ...
    TOlist = []
    tottlist = []
    for group in tqdm(grouplist, desc='Group', leave=False):
        list_of_smiles = dict_of_data[group]
        all_substr = []
        dict_substr = {}
        group_tot = 0
        for mol_smile in tqdm(list_of_smiles, desc='Molecule', leave=False):
